    ...
    if (fs.lstatSync(Options.audio).isDirectory()) {
        readdirSorted(Options.audio, { numeric: true }).then((filesSorted) => {
            ...
            if (mp3Files.length && wavFiles.length) {
                ...
                shell.exec(
                    `ffmpeg -f concat -safe 0 -i ${fileDir} -c copy ${Options.audioInput}`,
                    (err) => {
                        done(err);
                    }
                );
...
function execute(done, err) {
    ...
    shell.exec(
        `${ffmpegExe} -framerate ${
            Options.framerateIn
        } -pattern_type glob -i '${path.join(Options.images, "*.png")}' -i '${
            Options.audioInput
        }'  ${Options.framerateOut} -pix_fmt yuv420p ${Options.output}`,
        (err) => {
            done(err);
        }
    );
