...
function execute(done, err) {
    ...
    shell.exec(
        `${ffmpegExe} -framerate ${Options.framerateIn} -pattern_type glob -i '${path.join(Options.images, '*.png')}' -i '${Options.audioInput}'  ${Options.framerateOut} -pix_fmt yuv420p ${Options.output}`
    );
